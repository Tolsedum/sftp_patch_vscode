...
FileRewrite::FileRewrite(GeneralInformation &info){
    this->info = info;
    #ifdef WIN32
...
void FileRewrite::getSettingsPatch(std::string path_settings){
...
void FileRewrite::rewrite(){
    if(info.pos_begin > 0){
        ...
        if(is_opened){
            // Пишем во временный файл данный из файла с кодом до позиции вставки
